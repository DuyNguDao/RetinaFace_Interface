...
def plot_cm(CM, normalize=True, save_dir='', names_x=(), names_y=(), show=True):
    ...
    if True:
        ...
        labels_x = (0 < len(names_x) < 99) and len(names_x) == 2  # apply names to ticklabels
        labels_y = (0 < len(names_y) < 99) and len(names_y) == 2  # apply names to ticklabels
        with warnings.catch_warnings():
            warnings.simplefilter('ignore')  # suppress empty matrix RuntimeWarning: All-NaN slice encountered
            sn.heatmap(array, annot=2 < 30, annot_kws={"size": 20}, cmap='Blues', fmt=fmt, square=True,
